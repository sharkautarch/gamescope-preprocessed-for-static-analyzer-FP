# 1 "../subprojects/wlroots/types/xdg_shell/wlr_xdg_positioner.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/types/xdg_shell/wlr_xdg_positioner.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/types/xdg_shell/wlr_xdg_positioner.c" 2
...
# 3 "../subprojects/wlroots/types/xdg_shell/wlr_xdg_positioner.c" 2
...
# 4 "../subprojects/wlroots/types/xdg_shell/wlr_xdg_positioner.c" 2
...
# 5 "../subprojects/wlroots/types/xdg_shell/wlr_xdg_positioner.c" 2

static const struct xdg_positioner_interface xdg_positioner_implementation;

struct wlr_xdg_positioner *wlr_xdg_positioner_from_resource(
  struct wl_resource *resource) {
 ((wl_resource_instance_of(resource, &xdg_positioner_interface, &xdg_positioner_implementation)) ? (void) (0) : __assert_fail ("wl_resource_instance_of(resource, &xdg_positioner_interface, &xdg_positioner_implementation)", "types/xdg_shell/wlr_xdg_positioner.c", 11, __extension__ __PRETTY_FUNCTION__));
...
static void xdg_positioner_handle_set_parent_configure(
 ...
 positioner->rules.has_parent_configure_serial = 1;
...
static _Bool is_unconstrained(const struct constraint_offsets *offsets) {
...
static _Bool xdg_positioner_rules_unconstrain_by_flip(
  const struct wlr_xdg_positioner_rules *rules,
  const struct wlr_box *constraint, struct wlr_box *box,
 ...
 _Bool flip_x = ((offsets->left > 0) ^ (offsets->right > 0)) &&
  (rules->constraint_adjustment & XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_X);
 _Bool flip_y = ((offsets->top > 0) ^ (offsets->bottom > 0)) &&
...
static _Bool xdg_positioner_rules_unconstrain_by_slide(
  const struct wlr_xdg_positioner_rules *rules,
  const struct wlr_box *constraint, struct wlr_box *box,
  struct constraint_offsets *offsets) {
 uint32_t gravity = xdg_positioner_gravity_to_wlr_edges(rules->gravity);

 _Bool slide_x = (offsets->left > 0 || offsets->right > 0) &&
  (rules->constraint_adjustment & XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_SLIDE_X);
 _Bool slide_y = (offsets->top > 0 || offsets->bottom > 0) &&
...
# 398 "../subprojects/wlroots/types/xdg_shell/wlr_xdg_positioner.c"
...
static _Bool xdg_positioner_rules_unconstrain_by_resize(
  const struct wlr_xdg_positioner_rules *rules,
  const struct wlr_box *constraint, struct wlr_box *box,
  struct constraint_offsets *offsets) {
 _Bool resize_x = (offsets->left > 0 || offsets->right > 0) &&
  (rules->constraint_adjustment & XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_RESIZE_X);
 _Bool resize_y = (offsets->top > 0 || offsets->bottom > 0) &&
