# 1 "../subprojects/wlroots/render/drm_format_set.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/render/drm_format_set.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/render/drm_format_set.c" 2
...
# 3 "../subprojects/wlroots/render/drm_format_set.c" 2
# 1 "/usr/lib/clang/17/include/stdbool.h" 1 3
# 4 "../subprojects/wlroots/render/drm_format_set.c" 2
...
# 5 "../subprojects/wlroots/render/drm_format_set.c" 2
...
# 6 "../subprojects/wlroots/render/drm_format_set.c" 2
...
extern char *stpncpy (char *__restrict __dest,
        const char *__restrict __src, size_t __n)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2)));
# 7 "../subprojects/wlroots/render/drm_format_set.c" 2
...
_Bool wlr_drm_format_set_union(struct wlr_drm_format_set *dst,
 const struct wlr_drm_format_set *a, const struct wlr_drm_format_set *b);
# 8 "../subprojects/wlroots/render/drm_format_set.c" 2
# 1 "../subprojects/wlroots/include/wlr/util/log.h" 1
# 20 "../subprojects/wlroots/include/wlr/util/log.h"
...
# 9 "../subprojects/wlroots/render/drm_format_set.c" 2
...
# 10 "../subprojects/wlroots/render/drm_format_set.c" 2
...
_Bool wlr_drm_format_set_add(struct wlr_drm_format_set *set, uint32_t format,
  uint64_t modifier) {
 ((format != 0) ? (void) (0) : __assert_fail ("format != DRM_FORMAT_INVALID", "render/drm_format_set.c", 57, __extension__ __PRETTY_FUNCTION__));
 ...
 if (set->len == set->capacity) {
  ...
  if (!fmts) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "Allocation failed" ": %s", "render/drm_format_set.c", 76, strerror((*__errno_location ())));
   wlr_drm_format_finish(&fmt);
   return 0;
  }
...
_Bool wlr_drm_format_add(struct wlr_drm_format *fmt, uint64_t modifier) {
 ...
 if (fmt->len == fmt->capacity) {
  ...
  if (!new_modifiers) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "Allocation failed" ": %s", "render/drm_format_set.c", 114, strerror((*__errno_location ())));
   return 0;
  }
...
_Bool wlr_drm_format_copy(struct wlr_drm_format *dst, const struct wlr_drm_format *src) {
 ((src->len <= src->capacity) ? (void) (0) : __assert_fail ("src->len <= src->capacity", "render/drm_format_set.c", 127, __extension__ __PRETTY_FUNCTION__));
...
_Bool wlr_drm_format_intersect(struct wlr_drm_format *dst,
  const struct wlr_drm_format *a, const struct wlr_drm_format *b) {
 ((a->format == b->format) ? (void) (0) : __assert_fail ("a->format == b->format", "render/drm_format_set.c", 174, __extension__ __PRETTY_FUNCTION__));
 ...
 for (size_t i = 0; i < a->len; i++) {
  for (size_t j = 0; j < b->len; j++) {
   if (a->modifiers[i] == b->modifiers[j]) {
    ((fmt.len < fmt.capacity) ? (void) (0) : __assert_fail ("fmt.len < fmt.capacity", "render/drm_format_set.c", 192, __extension__ __PRETTY_FUNCTION__));
...
_Bool wlr_drm_format_set_intersect(struct wlr_drm_format_set *dst,
 ...
 if (out.formats == ((void*)0)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Allocation failed" ": %s", "render/drm_format_set.c", 210, strerror((*__errno_location ())));
  return 0;
 }
...
static _Bool drm_format_set_extend(struct wlr_drm_format_set *dst,
 ...
 for (size_t i = 0; i < src->len; i++) {
  ...
  for (size_t j = 0; j < format->len; j++) {
   if (!wlr_drm_format_set_add(dst, format->format, format->modifiers[j])) {
    _wlr_log(WLR_ERROR, "[%s:%d] " "Adding format/modifier to set failed" ": %s", "render/drm_format_set.c", 254, strerror((*__errno_location ())));
    return 0;
   }
  }
 }
...
_Bool wlr_drm_format_set_union(struct wlr_drm_format_set *dst,
 ...
 out.formats = malloc(sizeof(*out.formats) * out.capacity);
 if (out.formats == ((void*)0)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Allocation failed" ": %s", "render/drm_format_set.c", 269, strerror((*__errno_location ())));
  return 0;
 }
