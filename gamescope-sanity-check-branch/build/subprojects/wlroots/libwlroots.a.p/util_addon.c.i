# 1 "../subprojects/wlroots/util/addon.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/util/addon.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/util/addon.c" 2
...
# 3 "../subprojects/wlroots/util/addon.c" 2
...
extern char *stpncpy (char *__restrict __dest,
        const char *__restrict __src, size_t __n)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2)));
# 4 "../subprojects/wlroots/util/addon.c" 2
...
# 5 "../subprojects/wlroots/util/addon.c" 2
...
struct wlr_addon *wlr_addon_find(struct wlr_addon_set *set, const void *owner,
 const struct wlr_addon_interface *impl);
# 6 "../subprojects/wlroots/util/addon.c" 2
...
# 7 "../subprojects/wlroots/util/addon.c" 2
...
void wlr_addon_set_finish(struct wlr_addon_set *set) {
 while (!wl_list_empty(&set->addons)) {
  ...
  struct wlr_addon *addon = (__typeof__(addon))((char *)(link) - __builtin_offsetof(__typeof__(*addon), link));
  const struct wlr_addon_interface *impl = addon->impl;
  addon->impl->destroy(addon);
  if (set->addons.next == link) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "Dangling addon: %s", "util/addon.c", 20, impl->name);
   abort();
  }
 }
}
...
void wlr_addon_init(struct wlr_addon *addon, struct wlr_addon_set *set,
  const void *owner, const struct wlr_addon_interface *impl) {
 ((impl) ? (void) (0) : __assert_fail ("impl", "util/addon.c", 28, __extension__ __PRETTY_FUNCTION__));
 *addon = (struct wlr_addon){
  .impl = impl,
  .owner = owner,
 };
 struct wlr_addon *iter;
 for (iter = (__typeof__(iter))((char *)((&set->addons)->next) - __builtin_offsetof(__typeof__(*iter), link)); &iter->link != (&set->addons); iter = (__typeof__(iter))((char *)(iter->link.next) - __builtin_offsetof(__typeof__(*iter), link))) {
  if (iter->owner == addon->owner && iter->impl == addon->impl) {
   ((0 && "Can't have two addons of the same type with the same owner") ? (void) (0) : __assert_fail ("0 && \"Can't have two addons of the same type with the same owner\"", "util/addon.c", 36, __extension__ __PRETTY_FUNCTION__));
  }
 }
...
struct wlr_addon *wlr_addon_find(struct wlr_addon_set *set, const void *owner,
 ...
 for (addon = (__typeof__(addon))((char *)((&set->addons)->next) - __builtin_offsetof(__typeof__(*addon), link)); &addon->link != (&set->addons); addon = (__typeof__(addon))((char *)(addon->link.next) - __builtin_offsetof(__typeof__(*addon), link))) {
