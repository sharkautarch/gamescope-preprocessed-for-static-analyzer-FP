# 1 "../subprojects/wlroots/backend/headless/backend.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/backend/headless/backend.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/backend/headless/backend.c" 2
...
# 3 "../subprojects/wlroots/backend/headless/backend.c" 2
...
void wlr_output_send_request_state(struct wlr_output *output,
 const struct wlr_output_state *state);
# 4 "../subprojects/wlroots/backend/headless/backend.c" 2
...
# 5 "../subprojects/wlroots/backend/headless/backend.c" 2
# 1 "../subprojects/wlroots/include/backend/headless.h" 1
...
# 5 "../subprojects/wlroots/include/backend/headless.h" 2
...
# 6 "../subprojects/wlroots/include/backend/headless.h" 2
...
# 6 "../subprojects/wlroots/backend/headless/backend.c" 2

struct wlr_headless_backend *headless_backend_from_backend(
  struct wlr_backend *wlr_backend) {
 ((wlr_backend_is_headless(wlr_backend)) ? (void) (0) : __assert_fail ("wlr_backend_is_headless(wlr_backend)", "backend/headless/backend.c", 9, __extension__ __PRETTY_FUNCTION__));
 struct wlr_headless_backend *backend = (__typeof__(backend))((char *)(wlr_backend) - __builtin_offsetof(__typeof__(*backend), backend));
 return backend;
}
...
static _Bool backend_start(struct wlr_backend *wlr_backend) {
 ...
 _wlr_log(WLR_INFO, "[%s:%d] " "Starting headless backend", "backend/headless/backend.c", 17);
...
static void backend_destroy(struct wlr_backend *wlr_backend) {
 ...
 for (output = (__typeof__(output))((char *)((&backend->outputs)->next) - __builtin_offsetof(__typeof__(*output), link)), output_tmp = (__typeof__(output_tmp))((char *)((output)->link.next) - __builtin_offsetof(__typeof__(*output_tmp), link)); &output->link != (&backend->outputs); output = output_tmp, output_tmp = (__typeof__(output_tmp))((char *)(output->link.next) - __builtin_offsetof(__typeof__(*output_tmp), link))) {
...
struct wlr_backend *wlr_headless_backend_create(struct wl_event_loop *loop) {
 _wlr_log(WLR_INFO, "[%s:%d] " "Creating headless backend", "backend/headless/backend.c", 67);

 struct wlr_headless_backend *backend = calloc(1, sizeof(*backend));
 if (!backend) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to allocate wlr_headless_backend", "backend/headless/backend.c", 71);
  return ((void*)0);
 }
