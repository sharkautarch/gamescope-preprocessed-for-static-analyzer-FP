# 1 "../subprojects/wlroots/util/shm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/util/shm.c" 2
...
# 2 "../subprojects/wlroots/util/shm.c" 2
...
# 3 "../subprojects/wlroots/util/shm.c" 2
...
extern char *stpncpy (char *__restrict __dest,
        const char *__restrict __src, size_t __n)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2)));
# 4 "../subprojects/wlroots/util/shm.c" 2
...
# 5 "../subprojects/wlroots/util/shm.c" 2
...
# 6 "../subprojects/wlroots/util/shm.c" 2
...
extern int timespec_get (struct timespec *__ts, int __base)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1)));
# 7 "../subprojects/wlroots/util/shm.c" 2
...
# 10 "../subprojects/wlroots/util/shm.c" 2
...
static int excl_shm_open(char *name) {
 int retries = 100;
 do {
  randname(name + strlen("/wlroots-XXXXXX") - 6);

  --retries;

  int fd = shm_open(name, 02 | 0100 | 0200, 0600);
  if (fd >= 0) {
   return fd;
  }
 } while (retries > 0 && (*__errno_location ()) == 17);
...
_Bool allocate_shm_file_pair(size_t size, int *rw_fd_ptr, int *ro_fd_ptr) {
 char name[] = "/wlroots-XXXXXX";
