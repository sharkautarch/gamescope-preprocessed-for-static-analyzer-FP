# 1 "../subprojects/wlroots/xwayland/sockets.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 4 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 5 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 6 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 7 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 8 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 9 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 10 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 11 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 12 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 14 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 15 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 16 "../subprojects/wlroots/xwayland/sockets.c" 2
...
# 17 "../subprojects/wlroots/xwayland/sockets.c" 2
...
_Bool set_cloexec(int fd, _Bool cloexec) {
 ...
 if (flags == -1) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "fcntl failed" ": %s", "xwayland/sockets.c", 28, strerror((*__errno_location ())));
  return 0;
 }
 if (cloexec) {
  flags = flags | 1;
 ...
 if (fcntl(fd, 2, flags) == -1) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "fcntl failed" ": %s", "xwayland/sockets.c", 37, strerror((*__errno_location ())));
  return 0;
 }
 return 1;
}
...
static int open_socket(struct sockaddr_un *addr, size_t path_size) {
 int fd, rc;
 socklen_t size = __builtin_offsetof(struct sockaddr_un, sun_path) + path_size + 1;

 fd = socket(1, SOCK_STREAM, 0);
 if (fd < 0) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to create socket %c%s" ": %s", "xwayland/sockets.c", 51, addr->sun_path[0] ? addr->sun_path[0] : '@', addr->sun_path + 1, strerror((*__errno_location ())));
 ...
 if (bind(fd, (struct sockaddr*)addr, size) < 0) {
  rc = (*__errno_location ());
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to bind socket %c%s" ": %s", "xwayland/sockets.c", 66, addr->sun_path[0] ? addr->sun_path[0] : '@', addr->sun_path + 1, strerror((*__errno_location ())));
 ...
 if (listen(fd, 1) < 0) {
  rc = (*__errno_location ());
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to listen to socket %c%s" ": %s", "xwayland/sockets.c", 73, addr->sun_path[0] ? addr->sun_path[0] : '@', addr->sun_path + 1, strerror((*__errno_location ())));
...
static _Bool check_socket_dir(void) {
 struct stat buf;

 if (lstat(socket_dir, &buf)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to stat %s" ": %s", "xwayland/sockets.c", 92, socket_dir, strerror((*__errno_location ())));
  return 0;
 }
 if (!(buf.st_mode & 0040000)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "%s is not a directory", "xwayland/sockets.c", 96, socket_dir);
  return 0;
 }
 if (!((buf.st_uid == 0) || (buf.st_uid == getuid()))) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "%s not owned by root or us", "xwayland/sockets.c", 100, socket_dir);
  return 0;
 }
 if (!(buf.st_mode & 01000)) {

  if ((buf.st_mode & ((0200 >> 3) | ((0200 >> 3) >> 3)))) {

   _wlr_log(WLR_ERROR, "[%s:%d] " "sticky bit not set on %s", "xwayland/sockets.c", 107, socket_dir);
   return 0;
  }
 }
 return 1;
...
static _Bool open_sockets(int socks[2], int display) {
 struct sockaddr_un addr = { .sun_family = 1 };
 size_t path_size;

 if (mkdir(socket_dir, 0755) == 0) {
  _wlr_log(WLR_INFO, "[%s:%d] " "Created %s ourselves -- other users will " "be unable to create X11 UNIX sockets of their own", "xwayland/sockets.c", 121, socket_dir);


 } else if ((*__errno_location ()) != 17) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Unable to mkdir %s" ": %s", "xwayland/sockets.c", 123, socket_dir, strerror((*__errno_location ())));
  return 0;
...
int open_display_sockets(int socks[2]) {
 ...
 for (display = 0; display <= 32; display++) {
  ...
  if ((lock_fd = open(lock_name, 01 | 0100 | 0200 | 02000000, 0444)) >= 0) {
  ...
  if ((lock_fd = open(lock_name, 00 | 02000000)) < 0) {
   continue;
  }
  ...
  if (read_pid < 0 || read_pid > (2147483647) || end_pid != pid + sizeof(pid) - 2) {
   continue;
  }
  (*__errno_location ()) = 0;
  if (kill((pid_t)read_pid, 0) != 0 && (*__errno_location ()) == 3) {
 ...
 if (display > 32) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "No display available in the first 33", "xwayland/sockets.c", 217);
  return -1;
 }
