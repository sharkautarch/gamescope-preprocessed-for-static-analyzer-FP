# 1 "../subprojects/wlroots/render/allocator/allocator.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 3 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 4 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 5 "../subprojects/wlroots/render/allocator/allocator.c" 2
# 1 "subprojects/wlroots/include/wlr/config.h" 1
# 6 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 7 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
struct wlr_buffer *wlr_allocator_create_buffer(struct wlr_allocator *alloc,
 int width, int height, const struct wlr_drm_format *format);
# 8 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 9 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 10 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 11 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 12 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 13 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 14 "../subprojects/wlroots/render/allocator/allocator.c" 2
# 1 "../subprojects/wlroots/include/render/allocator/shm.h" 1



# 1 "../subprojects/wlroots/include/wlr/types/wlr_buffer.h" 1
# 5 "../subprojects/wlroots/include/render/allocator/shm.h" 2
...
# 15 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
# 16 "../subprojects/wlroots/render/allocator/allocator.c" 2
...
void wlr_allocator_init(struct wlr_allocator *alloc,
  const struct wlr_allocator_interface *impl, uint32_t buffer_caps) {
 ((impl && impl->destroy && impl->create_buffer) ? (void) (0) : __assert_fail ("impl && impl->destroy && impl->create_buffer", "render/allocator/allocator.c", 23, __extension__ __PRETTY_FUNCTION__));
...
static int reopen_drm_node(int drm_fd, _Bool allow_render_node) {
 if (drmIsMaster(drm_fd)) {

  uint32_t lessee_id;
  int lease_fd = drmModeCreateLease(drm_fd, ((void*)0), 0, 02000000, &lessee_id);
  if (lease_fd >= 0) {
   return lease_fd;
  } else if (lease_fd != -22 && lease_fd != -95) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "drmModeCreateLease failed" ": %s", "render/allocator/allocator.c", 42, strerror((*__errno_location ())));
   return -1;
  }
  _wlr_log(WLR_DEBUG, "[%s:%d] " "drmModeCreateLease failed, " "falling back to plain open", "render/allocator/allocator.c", 46);
 ...
 if (name == ((void*)0)) {
  ...
  if (name == ((void*)0)) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "drmGetDeviceNameFromFd2 failed", "render/allocator/allocator.c", 58);
   return -1;
  }
 }

 int new_fd = open(name, 02 | 02000000);
 if (new_fd < 0) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to open DRM node '%s'" ": %s", "render/allocator/allocator.c", 65, name, strerror((*__errno_location ())));
  free(name);
  return -1;
 }
 ...
 if (drmIsMaster(drm_fd) && drmGetNodeTypeFromFd(new_fd) == 0) {
  drm_magic_t magic;
  if (drmGetMagic(new_fd, &magic) < 0) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "drmGetMagic failed" ": %s", "render/allocator/allocator.c", 79, strerror((*__errno_location ())));
   close(new_fd);
   return -1;
  }

  if (drmAuthMagic(drm_fd, magic) < 0) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "drmAuthMagic failed" ": %s", "render/allocator/allocator.c", 85, strerror((*__errno_location ())));
   close(new_fd);
   return -1;
  }
 }
...
struct wlr_allocator *allocator_autocreate_with_drm_fd(
 ...
 struct wlr_allocator *alloc = ((void*)0);

 uint32_t gbm_caps = WLR_BUFFER_CAP_DMABUF;
 if ((backend_caps & gbm_caps) && (renderer_caps & gbm_caps)
   && drm_fd >= 0) {
# 116 "../subprojects/wlroots/render/allocator/allocator.c"
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Skipping gbm allocator: disabled at compile-time", "render/allocator/allocator.c", 116);

 }

 uint32_t shm_caps = WLR_BUFFER_CAP_SHM | WLR_BUFFER_CAP_DATA_PTR;
 if ((backend_caps & shm_caps) && (renderer_caps & shm_caps)) {
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Trying to create shm allocator", "render/allocator/allocator.c", 122);
  if ((alloc = wlr_shm_allocator_create()) != ((void*)0)) {
   return alloc;
  }
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Failed to create shm allocator", "render/allocator/allocator.c", 126);
 }
 ...
 if ((backend_caps & drm_caps) && (renderer_caps & drm_caps)
  ...
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Trying to create drm dumb allocator", "render/allocator/allocator.c", 132);
  int dumb_fd = reopen_drm_node(drm_fd, 0);
  if (dumb_fd < 0) {
   return ((void*)0);
  }
  if ((alloc = wlr_drm_dumb_allocator_create(dumb_fd)) != ((void*)0)) {
   return alloc;
  }
  close(dumb_fd);
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Failed to create drm dumb allocator", "render/allocator/allocator.c", 141);
 }

 _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to create allocator", "render/allocator/allocator.c", 144);
 return ((void*)0);
}
...
void wlr_allocator_destroy(struct wlr_allocator *alloc) {
 ...
 wl_signal_emit_mutable(&alloc->events.destroy, ((void*)0));
...
struct wlr_buffer *wlr_allocator_create_buffer(struct wlr_allocator *alloc,
 ...
 if (alloc->buffer_caps & WLR_BUFFER_CAP_DATA_PTR) {
  ((buffer->impl->begin_data_ptr_access && buffer->impl->end_data_ptr_access) ? (void) (0) : __assert_fail ("buffer->impl->begin_data_ptr_access && buffer->impl->end_data_ptr_access", "render/allocator/allocator.c", 177, __extension__ __PRETTY_FUNCTION__));

 }
 if (alloc->buffer_caps & WLR_BUFFER_CAP_DMABUF) {
  ((buffer->impl->get_dmabuf) ? (void) (0) : __assert_fail ("buffer->impl->get_dmabuf", "render/allocator/allocator.c", 180, __extension__ __PRETTY_FUNCTION__));
 }
 if (alloc->buffer_caps & WLR_BUFFER_CAP_SHM) {
  ((buffer->impl->get_shm) ? (void) (0) : __assert_fail ("buffer->impl->get_shm", "render/allocator/allocator.c", 183, __extension__ __PRETTY_FUNCTION__));
 }
 return buffer;
}
