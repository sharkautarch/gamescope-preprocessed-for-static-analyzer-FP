# 1 "../subprojects/wlroots/backend/headless/output.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/backend/headless/output.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/backend/headless/output.c" 2
...
# 3 "../subprojects/wlroots/backend/headless/output.c" 2
...
# 4 "../subprojects/wlroots/backend/headless/output.c" 2
...
void wlr_output_send_request_state(struct wlr_output *output,
 const struct wlr_output_state *state);
# 5 "../subprojects/wlroots/backend/headless/output.c" 2
...
# 6 "../subprojects/wlroots/backend/headless/output.c" 2
...
# 7 "../subprojects/wlroots/backend/headless/output.c" 2
...
# 8 "../subprojects/wlroots/backend/headless/output.c" 2
...
# 9 "../subprojects/wlroots/backend/headless/output.c" 2
...
static struct wlr_headless_output *headless_output_from_output(
  struct wlr_output *wlr_output) {
 ((wlr_output_is_headless(wlr_output)) ? (void) (0) : __assert_fail ("wlr_output_is_headless(wlr_output)", "backend/headless/output.c", 20, __extension__ __PRETTY_FUNCTION__));
 struct wlr_headless_output *output = (__typeof__(output))((char *)(wlr_output) - __builtin_offsetof(__typeof__(*output), wlr_output));
 return output;
}
...
static _Bool output_test(struct wlr_output *wlr_output,
 ...
 if (unsupported != 0) {
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Unsupported output state fields: 0x%""x", "backend/headless/output.c", 39, unsupported);

  return 0;
 }

 if (state->committed & WLR_OUTPUT_STATE_MODE) {
  ((state->mode_type == WLR_OUTPUT_STATE_MODE_CUSTOM) ? (void) (0) : __assert_fail ("state->mode_type == WLR_OUTPUT_STATE_MODE_CUSTOM", "backend/headless/output.c", 44, __extension__ __PRETTY_FUNCTION__));
 }

 if (state->committed & WLR_OUTPUT_STATE_LAYERS) {
  for (size_t i = 0; i < state->layers_len; i++) {
   state->layers[i].accepted = 1;
  }
 }
...
_Bool wlr_output_is_headless(struct wlr_output *wlr_output) {
...
struct wlr_output *wlr_headless_add_output(struct wlr_backend *wlr_backend,
 ...
 if (output == ((void*)0)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to allocate wlr_headless_output", "backend/headless/output.c", 112);
  return ((void*)0);
 }
