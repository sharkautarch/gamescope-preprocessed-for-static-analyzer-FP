# 1 "../subprojects/wlroots/xwayland/selection/incoming.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
# 3 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
# 4 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
extern char *stpncpy (char *__restrict __dest,
        const char *__restrict __src, size_t __n)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1, 2)));
# 5 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
# 6 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
void wlr_data_source_dnd_action(struct wlr_data_source *source,
 enum wl_data_device_manager_dnd_action action);
# 7 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
void wlr_seat_set_primary_selection(struct wlr_seat *seat,
 struct wlr_primary_selection_source *source, uint32_t serial);
# 8 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
# 9 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
xcb_xfixes_get_client_disconnect_mode_reply (xcb_connection_t *c,
                                             ...
                                             xcb_generic_error_t **e);
# 10 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
# 11 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
# 12 "../subprojects/wlroots/xwayland/selection/incoming.c" 2
...
xwm_selection_find_incoming_transfer_by_window(
  struct wlr_xwm_selection *selection, xcb_window_t window) {
 struct wlr_xwm_selection_transfer *transfer;
 for (transfer = (__typeof__(transfer))((char *)((&selection->incoming)->next) - __builtin_offsetof(__typeof__(*transfer), link)); &transfer->link != (&selection->incoming); transfer = (__typeof__(transfer))((char *)(transfer->link.next) - __builtin_offsetof(__typeof__(*transfer), link))) {
...
static _Bool xwm_selection_transfer_get_incoming_selection_property(
  struct wlr_xwm_selection_transfer *transfer, _Bool delete) {
 ...
 if (!transfer->property_reply) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "cannot get selection property", "xwayland/selection/incoming.c", 77);
  return 0;
 }
...
static void xwm_notify_ready_for_next_incr_chunk(
  struct wlr_xwm_selection_transfer *transfer) {
 struct wlr_xwm *xwm = transfer->selection->xwm;
 ((transfer->incr) ? (void) (0) : __assert_fail ("transfer->incr", "xwayland/selection/incoming.c", 87, __extension__ __PRETTY_FUNCTION__));

 _wlr_log(WLR_DEBUG, "[%s:%d] " "deleting property", "xwayland/selection/incoming.c", 89);
...
static int write_selection_property_to_wl_client(int fd, uint32_t mask,
 ...
 if (len == -1) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "write error to target fd %d" ": %s", "xwayland/selection/incoming.c", 112, fd, strerror((*__errno_location ())));
  xwm_selection_transfer_destroy(transfer);
  return 0;
 }

 _wlr_log(WLR_DEBUG, "[%s:%d] " "wrote %zd (total %zd, remaining %d) of %d bytes to fd %d", "xwayland/selection/incoming.c", 120, len, transfer->property_start + len, remainder, xcb_get_property_value_length(transfer->property_reply), fd);
 ...
 if (len < remainder) {
  ...
  _wlr_log(WLR_DEBUG, "[%s:%d] " "transfer complete", "xwayland/selection/incoming.c", 128);
  xwm_selection_transfer_destroy(transfer);
 }
...
static void xwm_write_selection_property_to_wl_client(
 ...
 _Bool wl_client_finished_consuming = !write_selection_property_to_wl_client(
...
void xwm_get_incr_chunk(struct wlr_xwm_selection_transfer *transfer) {
 _wlr_log(WLR_DEBUG, "[%s:%d] " "xwm_get_incr_chunk", "xwayland/selection/incoming.c", 159);

 if (transfer->property_reply) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "X11 client offered a new property before we deleted", "xwayland/selection/incoming.c", 162);
  return;
 }

 if (!xwm_selection_transfer_get_incoming_selection_property(transfer, 0)) {
  return;
 }
 ...
 if (xcb_get_property_value_length(transfer->property_reply) > 0) {
  ...
  _wlr_log(WLR_DEBUG, "[%s:%d] " "incremental transfer complete", "xwayland/selection/incoming.c", 173);
  xwm_selection_transfer_destroy(transfer);
 }
}
...
static void xwm_selection_transfer_get_data(
 ...
 if (!xwm_selection_transfer_get_incoming_selection_property(transfer, 1)) {
  return;
 }
...
static void source_send(struct wlr_xwm_selection *selection,
 ...
 if (!found) {
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Cannot send X11 selection to Wayland: " "unsupported MIME type", "xwayland/selection/incoming.c", 217);
 ...
 if (!transfer) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Cannot create transfer", "xwayland/selection/incoming.c", 225);
  close(fd);
  return;
 }
...
static struct x11_data_source *data_source_from_wlr_data_source(
  struct wlr_data_source *wlr_source) {
 ((data_source_is_xwayland(wlr_source)) ? (void) (0) : __assert_fail ("data_source_is_xwayland(wlr_source)", "xwayland/selection/incoming.c", 258, __extension__ __PRETTY_FUNCTION__));
 struct x11_data_source *source = (__typeof__(source))((char *)(wlr_source) - __builtin_offsetof(__typeof__(*source), base));
 return source;
}
...
static void primary_selection_source_send(
  struct wlr_primary_selection_source *wlr_source,
  const char *mime_type, int fd) {
 struct x11_primary_selection_source *source = (__typeof__(source))((char *)(wlr_source) - __builtin_offsetof(__typeof__(*source), base));
 struct wlr_xwm_selection *selection = source->selection;
...
static _Bool source_get_targets(struct wlr_xwm_selection *selection,
  struct wl_array *mime_types, struct wl_array *mime_types_atoms) {
 struct wlr_xwm *xwm = selection->xwm;
...
static void xwm_selection_get_targets(struct wlr_xwm_selection *selection) {
 ...
 if (selection == &xwm->clipboard_selection) {
  ...
  _Bool ok = source_get_targets(selection, &source->base.mime_types,
   &source->mime_types_atoms);
  if (ok) {
   wlr_seat_request_set_selection(xwm->seat, ((void*)0), &source->base,
...
void xwm_handle_selection_notify(struct wlr_xwm *xwm,
  xcb_selection_notify_event_t *event) {
 _wlr_log(WLR_DEBUG, "[%s:%d] " "XCB_SELECTION_NOTIFY (selection=%u, property=%u, target=%u)", "xwayland/selection/incoming.c", 449, event->selection, event->property, event->target);
 ...
 if (event->property == XCB_ATOM_NONE) {
  if (transfer) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "convert selection failed", "xwayland/selection/incoming.c", 462);
   xwm_selection_transfer_destroy(transfer);
  }
 } else if (event->target == xwm->atoms[TARGETS]) {

  if (xwm->focus_surface == ((void*)0)) {
   _wlr_log(WLR_DEBUG, "[%s:%d] " "denying write access to clipboard: " "no xwayland surface focused", "xwayland/selection/incoming.c", 469);
...
int xwm_handle_xfixes_selection_notify(struct wlr_xwm *xwm,
  xcb_xfixes_selection_notify_event_t *event) {
 _wlr_log(WLR_DEBUG, "[%s:%d] " "XCB_XFIXES_SELECTION_NOTIFY (selection=%u, owner=%u)", "xwayland/selection/incoming.c", 483, event->selection, event->owner);
 ...
 if (event->owner == XCB_WINDOW_NONE) {
  if (selection->owner != selection->window) {
   ...
   if (selection == &xwm->clipboard_selection) {
    wlr_seat_request_set_selection(xwm->seat, ((void*)0), ((void*)0),
     wl_display_next_serial(xwm->xwayland->wl_display));
   } else if (selection == &xwm->primary_selection) {
    wlr_seat_request_set_primary_selection(xwm->seat, ((void*)0), ((void*)0),
     wl_display_next_serial(xwm->xwayland->wl_display));
   } else if (selection == &xwm->dnd_selection) {

   } else {
    _wlr_log(WLR_DEBUG, "[%s:%d] " "X11 selection has been cleared, but cannot " "clear Wayland selection", "xwayland/selection/incoming.c", 504);
 ...
 if (selection->owner == selection->window &&
  ...
  _wlr_log(WLR_DEBUG, "[%s:%d] " "proxy window lost selection ownership", "xwayland/selection/incoming.c", 514);
 }
