# 1 "../subprojects/wlroots/render/swapchain.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/render/swapchain.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/render/swapchain.c" 2
...
# 3 "../subprojects/wlroots/render/swapchain.c" 2
...
# 4 "../subprojects/wlroots/render/swapchain.c" 2
...
# 6 "../subprojects/wlroots/include/wlr/render/swapchain.h" 2
...
void wlr_swapchain_set_buffer_submitted(struct wlr_swapchain *swapchain,
 struct wlr_buffer *buffer);
# 5 "../subprojects/wlroots/render/swapchain.c" 2
...
# 6 "../subprojects/wlroots/render/swapchain.c" 2
...
# 7 "../subprojects/wlroots/render/swapchain.c" 2
...
# 8 "../subprojects/wlroots/render/swapchain.c" 2

static void swapchain_handle_allocator_destroy(struct wl_listener *listener,
  void *data) {
 struct wlr_swapchain *swapchain =
  (__typeof__(swapchain))((char *)(listener) - __builtin_offsetof(__typeof__(*swapchain), allocator_destroy));
 swapchain->allocator = ((void*)0);
...
static void slot_handle_release(struct wl_listener *listener, void *data) {
 struct wlr_swapchain_slot *slot =
  (__typeof__(slot))((char *)(listener) - __builtin_offsetof(__typeof__(*slot), release));
 wl_list_remove(&slot->release.link);
 slot->acquired = 0;
}
...
static struct wlr_buffer *slot_acquire(struct wlr_swapchain *swapchain,
 ...
 ((!slot->acquired) ? (void) (0) : __assert_fail ("!slot->acquired", "render/swapchain.c", 69, __extension__ __PRETTY_FUNCTION__));
 ((slot->buffer != ((void*)0)) ? (void) (0) : __assert_fail ("slot->buffer != NULL", "render/swapchain.c", 70, __extension__ __PRETTY_FUNCTION__));
...
struct wlr_buffer *wlr_swapchain_acquire(struct wlr_swapchain *swapchain,
  int *age) {
 struct wlr_swapchain_slot *free_slot = ((void*)0);
 for (size_t i = 0; i < 4; i++) {
  struct wlr_swapchain_slot *slot = &swapchain->slots[i];
  if (slot->acquired) {
   continue;
  }
  if (slot->buffer != ((void*)0)) {
   return slot_acquire(swapchain, slot, age);
  }
  free_slot = slot;
 }
 if (free_slot == ((void*)0)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "No free output buffer slot", "render/swapchain.c", 98);
  return ((void*)0);
 }

 if (swapchain->allocator == ((void*)0)) {
  return ((void*)0);
 }

 _wlr_log(WLR_DEBUG, "[%s:%d] " "Allocating new swapchain buffer", "render/swapchain.c", 106);
 free_slot->buffer = wlr_allocator_create_buffer(swapchain->allocator,
  swapchain->width, swapchain->height, &swapchain->format);
 if (free_slot->buffer == ((void*)0)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to allocate buffer", "render/swapchain.c", 110);
  return ((void*)0);
 }
...
void wlr_swapchain_set_buffer_submitted(struct wlr_swapchain *swapchain,
  struct wlr_buffer *buffer) {
 ((buffer != ((void*)0)) ? (void) (0) : __assert_fail ("buffer != NULL", "render/swapchain.c", 129, __extension__ __PRETTY_FUNCTION__));
