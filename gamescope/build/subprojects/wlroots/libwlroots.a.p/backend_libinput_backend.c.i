# 1 "../subprojects/wlroots/backend/libinput/backend.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/backend/libinput/backend.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/backend/libinput/backend.c" 2
...
# 3 "../subprojects/wlroots/backend/libinput/backend.c" 2
...
# 5 "../subprojects/wlroots/backend/libinput/backend.c" 2
# 1 "../subprojects/wlroots/include/wlr/backend/interface.h" 1
# 12 "../subprojects/wlroots/include/wlr/backend/interface.h"
# 1 "/usr/lib/clang/17/include/stdbool.h" 1 3
# 13 "../subprojects/wlroots/include/wlr/backend/interface.h" 2
...
# 32 "/usr/include/wayland-server-core.h" 3 4
...
# 6 "../subprojects/wlroots/backend/libinput/backend.c" 2
...
# 7 "../subprojects/wlroots/backend/libinput/backend.c" 2
...
# 8 "../subprojects/wlroots/backend/libinput/backend.c" 2
...
void handle_tablet_pad_strip(struct libinput_event *event,
 struct wlr_tablet_pad *tablet_pad);
# 9 "../subprojects/wlroots/backend/libinput/backend.c" 2
...
# 10 "../subprojects/wlroots/backend/libinput/backend.c" 2

static struct wlr_libinput_backend *get_libinput_backend_from_backend(
  struct wlr_backend *wlr_backend) {
 ((wlr_backend_is_libinput(wlr_backend)) ? (void) (0) : __assert_fail ("wlr_backend_is_libinput(wlr_backend)", "backend/libinput/backend.c", 13, __extension__ __PRETTY_FUNCTION__));
 struct wlr_libinput_backend *backend = (__typeof__(backend))((char *)(wlr_backend) - __builtin_offsetof(__typeof__(*backend), backend));
 return backend;
}
...
static void libinput_close_restricted(int fd, void *_backend) {
 ...
 _Bool found = 0;
 for (dev = (__typeof__(dev))((char *)((&backend->session->devices)->next) - __builtin_offsetof(__typeof__(*dev), link)); &dev->link != (&backend->session->devices); dev = (__typeof__(dev))((char *)(dev->link.next) - __builtin_offsetof(__typeof__(*dev), link))) {
  if (dev->fd == fd) {
   found = 1;
   break;
  }
...
static int handle_libinput_readable(int fd, uint32_t mask, void *_backend) {
 ...
 if (ret != 0) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to dispatch libinput: %s", "backend/libinput/backend.c", 53, strerror(-ret));
...
static _Bool backend_start(struct wlr_backend *wlr_backend) {
 struct wlr_libinput_backend *backend =
  get_libinput_backend_from_backend(wlr_backend);
 _wlr_log(WLR_DEBUG, "[%s:%d] " "Starting libinput backend", "backend/libinput/backend.c", 88);

 backend->libinput_context = libinput_udev_create_context(&libinput_impl,
  backend, backend->session->udev);
 if (!backend->libinput_context) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to create libinput context", "backend/libinput/backend.c", 93);
  return 0;
 }

 if (libinput_udev_assign_seat(backend->libinput_context,
   backend->session->seat) != 0) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to assign libinput seat", "backend/libinput/backend.c", 99);
  return 0;
 }
 ...
 if (!env_parse_bool("WLR_LIBINPUT_NO_DEVICES") && wl_list_empty(&backend->devices)) {
  ...
  if (wl_list_empty(&backend->devices)) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "libinput initialization failed, no input devices", "backend/libinput/backend.c", 112);
   _wlr_log(WLR_ERROR, "[%s:%d] " "Set WLR_LIBINPUT_NO_DEVICES=1 to suppress this check", "backend/libinput/backend.c", 113);
   return 0;
  }
 }
 ...
 if (!backend->input_event) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to create input event on event loop", "backend/libinput/backend.c", 124);
  return 0;
 }
 _wlr_log(WLR_DEBUG, "[%s:%d] " "libinput successfully initialized", "backend/libinput/backend.c", 127);
 return 1;
}
...
static void backend_destroy(struct wlr_backend *wlr_backend) {
 ...
 for (dev = (__typeof__(dev))((char *)((&backend->devices)->next) - __builtin_offsetof(__typeof__(*dev), link)), tmp = (__typeof__(tmp))((char *)((dev)->link.next) - __builtin_offsetof(__typeof__(*tmp), link)); &dev->link != (&backend->devices); dev = tmp, tmp = (__typeof__(tmp))((char *)(dev->link.next) - __builtin_offsetof(__typeof__(*tmp), link))) {
...
_Bool wlr_backend_is_libinput(struct wlr_backend *b) {
 return b->impl == &backend_impl;
}

static void session_signal(struct wl_listener *listener, void *data) {
 struct wlr_libinput_backend *backend =
  (__typeof__(backend))((char *)(listener) - __builtin_offsetof(__typeof__(*backend), session_signal));
...
static void handle_session_destroy(struct wl_listener *listener, void *data) {
 struct wlr_libinput_backend *backend =
  (__typeof__(backend))((char *)(listener) - __builtin_offsetof(__typeof__(*backend), session_destroy));
...
struct wlr_backend *wlr_libinput_backend_create(struct wlr_session *session) {
 ...
 if (!backend) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Allocation failed: %s", "backend/libinput/backend.c", 189, strerror((*__errno_location ())));
  return ((void*)0);
 }
...
struct libinput_device *wlr_libinput_get_device_handle(
 ...
 struct wlr_libinput_input_device *dev = ((void*)0);
 switch (wlr_dev->type) {
