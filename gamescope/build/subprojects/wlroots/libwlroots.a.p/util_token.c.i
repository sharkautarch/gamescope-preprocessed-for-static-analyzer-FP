...
# 3 "../subprojects/wlroots/util/token.c" 2
...
# 5 "../subprojects/wlroots/util/token.c" 2
...
# 6 "../subprojects/wlroots/util/token.c" 2
...
# 8 "../subprojects/wlroots/util/token.c" 2
...
# 10 "../subprojects/wlroots/util/token.c" 2
...
# 11 "../subprojects/wlroots/util/token.c" 2
...
# 12 "../subprojects/wlroots/util/token.c" 2

_Bool generate_token(char out[static 33]) {
 static FILE *urandom = ((void*)0);
 uint64_t data[2];

 if (!urandom) {
  int fd = open("/dev/urandom", 00 | 02000000);
  if (fd < 0) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to open random device" ": %s", "util/token.c", 20, strerror((*__errno_location ())));
   return 0;
  }
  if (!(urandom = fdopen(fd, "r"))) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "fdopen failed" ": %s", "util/token.c", 24, strerror((*__errno_location ())));
   close(fd);
   return 0;
  }
 }
 if (fread(data, sizeof(data), 1, urandom) != 1) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to read from random device" ": %s", "util/token.c", 30, strerror((*__errno_location ())));
  return 0;
 }
 if (snprintf(out, 33, "%016" "l" "x" "%016" "l" "x", data[0], data[1]) != 33 - 1) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to format hex string token" ": %s", "util/token.c", 34, strerror((*__errno_location ())));
  return 0;
 }
 return 1;
}
