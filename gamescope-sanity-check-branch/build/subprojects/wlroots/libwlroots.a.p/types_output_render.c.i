# 1 "../subprojects/wlroots/types/output/render.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/types/output/render.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/types/output/render.c" 2
...
# 3 "../subprojects/wlroots/types/output/render.c" 2
...
# 4 "../subprojects/wlroots/types/output/render.c" 2
...
void wlr_output_send_request_state(struct wlr_output *output,
 const struct wlr_output_state *state);
# 5 "../subprojects/wlroots/types/output/render.c" 2
...
void *wlr_texture_read_pixel_options_get_data(
 const struct wlr_texture_read_pixels_options *options);
# 6 "../subprojects/wlroots/types/output/render.c" 2
...
void wlr_swapchain_set_buffer_submitted(struct wlr_swapchain *swapchain,
 struct wlr_buffer *buffer);
# 7 "../subprojects/wlroots/types/output/render.c" 2
...
# 8 "../subprojects/wlroots/types/output/render.c" 2
...
# 9 "../subprojects/wlroots/types/output/render.c" 2
...
# 10 "../subprojects/wlroots/types/output/render.c" 2
...
# 11 "../subprojects/wlroots/types/output/render.c" 2
...
# 12 "../subprojects/wlroots/types/output/render.c" 2
...
# 13 "../subprojects/wlroots/types/output/render.c" 2
...
# 14 "../subprojects/wlroots/types/output/render.c" 2
...
# 15 "../subprojects/wlroots/types/output/render.c" 2

_Bool wlr_output_init_render(struct wlr_output *output,
  struct wlr_allocator *allocator, struct wlr_renderer *renderer) {
 ((allocator != ((void*)0) && renderer != ((void*)0)) ? (void) (0) : __assert_fail ("allocator != NULL && renderer != NULL", "types/output/render.c", 18, __extension__ __PRETTY_FUNCTION__));
 ...
 if (!(backend_caps & allocator->buffer_caps)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "output backend and allocator buffer capabilities " "don't match", "types/output/render.c", 25);

  return 0;
 } else if (!(renderer_caps & allocator->buffer_caps)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "renderer and allocator buffer capabilities " "don't match", "types/output/render.c", 29);
...
static struct wlr_buffer *output_acquire_empty_buffer(struct wlr_output *output,
  const struct wlr_output_state *state) {
 ((!(state->committed & WLR_OUTPUT_STATE_BUFFER)) ? (void) (0) : __assert_fail ("!(state->committed & WLR_OUTPUT_STATE_BUFFER)", "types/output/render.c", 47, __extension__ __PRETTY_FUNCTION__));
 ...
 struct wlr_buffer *buffer = wlr_swapchain_acquire(output->swapchain, ((void*)0));
 if (buffer == ((void*)0)) {
  return ((void*)0);
 }

 struct wlr_render_pass *pass =
  wlr_renderer_begin_buffer_pass(output->renderer, buffer, ((void*)0));
 if (pass == ((void*)0)) {
...
_Bool output_ensure_buffer(struct wlr_output *output,
  struct wlr_output_state *state, _Bool *new_buffer) {
 ((*new_buffer == 0) ? (void) (0) : __assert_fail ("*new_buffer == false", "types/output/render.c", 87, __extension__ __PRETTY_FUNCTION__));
 ...
 if (output->renderer == ((void*)0)) {
  return 1;
 }

 _Bool enabled = output->enabled;
 ...
 _wlr_log(WLR_DEBUG, "[%s:%d] " "Attaching empty buffer to output for modeset", "types/output/render.c", 127);
...
void wlr_output_lock_attach_render(struct wlr_output *output, _Bool lock) {
 if (lock) {
  ...
  ((output->attach_render_locks > 0) ? (void) (0) : __assert_fail ("output->attach_render_locks > 0", "types/output/render.c", 143, __extension__ __PRETTY_FUNCTION__));
  --output->attach_render_locks;
 }
 _wlr_log(WLR_DEBUG, "[%s:%d] " "%s direct scan-out on output '%s' (locks: %d)", "types/output/render.c", 148, lock ? "Disabling" : "Enabling", output->name, output->attach_render_locks);
...
_Bool output_pick_format(struct wlr_output *output,
 ...
 ((renderer != ((void*)0) && allocator != ((void*)0)) ? (void) (0) : __assert_fail ("renderer != NULL && allocator != NULL", "types/output/render.c", 156, __extension__ __PRETTY_FUNCTION__));

 const struct wlr_drm_format_set *render_formats =
  wlr_renderer_get_render_formats(renderer);
 if (render_formats == ((void*)0)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to get render formats", "types/output/render.c", 161);
  return 0;
 }

 const struct wlr_drm_format *render_format =
  wlr_drm_format_set_get(render_formats, fmt);
 if (render_format == ((void*)0)) {
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Renderer doesn't support format 0x%""X", "types/output/render.c", 168, fmt);
  return 0;
 }

 if (display_formats != ((void*)0)) {
  const struct wlr_drm_format *display_format =
   wlr_drm_format_set_get(display_formats, fmt);
  if (display_format == ((void*)0)) {
   _wlr_log(WLR_DEBUG, "[%s:%d] " "Output doesn't support format 0x%""X", "types/output/render.c", 176, fmt);
   return 0;
  }
  if (!wlr_drm_format_intersect(format, display_format, render_format)) {
   _wlr_log(WLR_DEBUG, "[%s:%d] " "Failed to intersect display and render " "modifiers for format 0x%""X" " on output %s", "types/output/render.c", 182, fmt, output->name);
 ...
 if (format->len == 0) {
  ...
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Failed to pick output format", "types/output/render.c", 194);
  return 0;
 }
...
struct wlr_render_pass *wlr_output_begin_render_pass(struct wlr_output *output,
 ...
 struct wlr_renderer *renderer = output->renderer;
 ((renderer != ((void*)0)) ? (void) (0) : __assert_fail ("renderer != NULL", "types/output/render.c", 214, __extension__ __PRETTY_FUNCTION__));
