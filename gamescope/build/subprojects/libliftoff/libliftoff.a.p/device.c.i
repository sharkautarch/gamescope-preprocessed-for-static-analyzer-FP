# 1 "../subprojects/libliftoff/device.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/libliftoff/device.c" 2
...
# 2 "../subprojects/libliftoff/device.c" 2
...
# 3 "../subprojects/libliftoff/device.c" 2
...
# 4 "../subprojects/libliftoff/device.c" 2
...
# 5 "../subprojects/libliftoff/device.c" 2
...
# 6 "../subprojects/libliftoff/device.c" 2
...
# 7 "../subprojects/libliftoff/device.c" 2
...
liftoff_device_destroy(struct liftoff_device *device)
{
 ...
 close(device->drm_fd);
 for (plane = (__typeof__(plane))((char *)((&device->planes)->next) - __builtin_offsetof(__typeof__(*plane), link)), tmp = (__typeof__(tmp))((char *)(plane->link.next) - __builtin_offsetof(__typeof__(*tmp), link)); &plane->link != (&device->planes); plane = tmp, tmp = (__typeof__(tmp))((char *)(plane->link.next) - __builtin_offsetof(__typeof__(*tmp), link))) {
...
liftoff_device_register_all_planes(struct liftoff_device *device)
{
 ...
 for (i = 0; i < drm_plane_res->count_planes; i++) {
  if (liftoff_plane_create(device, drm_plane_res->planes[i]) == ((void*)0)) {
   return -(*__errno_location ());
  }
 }
 drmModeFreePlaneResources(drm_plane_res);
