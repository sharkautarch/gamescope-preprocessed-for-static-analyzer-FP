# 1 "../subprojects/libliftoff/output.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/libliftoff/output.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/libliftoff/output.c" 2
...
# 3 "../subprojects/libliftoff/output.c" 2
...
# 4 "../subprojects/libliftoff/output.c" 2
...
# 5 "../subprojects/libliftoff/output.c" 2
...
# 6 "../subprojects/libliftoff/output.c" 2
...
# 7 "../subprojects/libliftoff/output.c" 2
...
liftoff_output_set_composition_layer(struct liftoff_output *output,
         struct liftoff_layer *layer)
{
 ((layer->output == output) ? (void) (0) : __assert_fail ("layer->output == output", "../subprojects/libliftoff/output.c", 53, __extension__ __PRETTY_FUNCTION__));
...
output_log_layers(struct liftoff_output *output)
{
 struct liftoff_layer *layer;
 size_t i;
 _Bool is_composition_layer;

 if (!log_has(LIFTOFF_DEBUG)) {
  return;
 }

 liftoff_log(LIFTOFF_DEBUG, "Layers on CRTC %""u"" (%zu total):",
 ...
 for (layer = (__typeof__(layer))((char *)((&output->layers)->next) - __builtin_offsetof(__typeof__(*layer), link)); &layer->link != (&output->layers); layer = (__typeof__(layer))((char *)(layer->link.next) - __builtin_offsetof(__typeof__(*layer), link))) {
  ...
  for (i = 0; i < layer->props_len; i++) {
   ...
   if (strcmp(name, "CRTC_X") == 0 ||
    ...
    liftoff_log(LIFTOFF_DEBUG, "    %s = %""l" "u",
         name, value);
   }
  }
 }
