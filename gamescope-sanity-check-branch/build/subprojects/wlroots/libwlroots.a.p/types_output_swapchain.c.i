# 1 "../subprojects/wlroots/types/output/swapchain.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 382 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../subprojects/wlroots/types/output/swapchain.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 2 "../subprojects/wlroots/types/output/swapchain.c" 2
...
# 3 "../subprojects/wlroots/types/output/swapchain.c" 2
...
# 4 "../subprojects/wlroots/types/output/swapchain.c" 2
# 1 "../subprojects/wlroots/include/wlr/render/allocator.h" 1
# 12 "../subprojects/wlroots/include/wlr/render/allocator.h"
...
# 13 "../subprojects/wlroots/include/wlr/render/allocator.h" 2
...
struct wlr_buffer *wlr_allocator_create_buffer(struct wlr_allocator *alloc,
 int width, int height, const struct wlr_drm_format *format);
# 5 "../subprojects/wlroots/types/output/swapchain.c" 2
...
void wlr_swapchain_set_buffer_submitted(struct wlr_swapchain *swapchain,
 struct wlr_buffer *buffer);
# 6 "../subprojects/wlroots/types/output/swapchain.c" 2
...
# 7 "../subprojects/wlroots/types/output/swapchain.c" 2
...
# 8 "../subprojects/wlroots/types/output/swapchain.c" 2
...
# 10 "../subprojects/wlroots/types/output/swapchain.c" 2
...
# 11 "../subprojects/wlroots/types/output/swapchain.c" 2

static struct wlr_swapchain *create_swapchain(struct wlr_output *output,
  int width, int height, uint32_t render_format, _Bool allow_modifiers) {
 struct wlr_allocator *allocator = output->allocator;
 ((output->allocator != ((void*)0)) ? (void) (0) : __assert_fail ("output->allocator != NULL", "types/output/swapchain.c", 15, __extension__ __PRETTY_FUNCTION__));
 ...
 if (!output_pick_format(output, display_formats, &format, render_format)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to pick primary buffer format for output '%s'", "types/output/swapchain.c", 22, output->name);

  return ((void*)0);
 }

 char *format_name = drmGetFormatName(format.format);
 _wlr_log(WLR_DEBUG, "[%s:%d] " "Choosing primary buffer format %s (0x%08""X"") for output '%s'", "types/output/swapchain.c", 28, format_name ? format_name : "<unknown>", format.format, output->name);

 free(format_name);

 if (!allow_modifiers && (format.len != 1 || format.modifiers[0] != ((((__u64)0) << 56) | ((0) & 0x00ffffffffffffffULL)))) {
  if (!wlr_drm_format_has(&format, ((((__u64)0) << 56) | ((((1ULL << 56) - 1)) & 0x00ffffffffffffffULL)))) {
   _wlr_log(WLR_DEBUG, "[%s:%d] " "Implicit modifiers not supported", "types/output/swapchain.c", 33);
  ...
  if (!wlr_drm_format_add(&format, ((((__u64)0) << 56) | ((((1ULL << 56) - 1)) & 0x00ffffffffffffffULL)))) {
   _wlr_log(WLR_DEBUG, "[%s:%d] " "Failed to add implicit modifier to format", "types/output/swapchain.c", 40);
   wlr_drm_format_finish(&format);
   return ((void*)0);
  }
 }
...
static _Bool test_swapchain(struct wlr_output *output,
 ...
 copy.buffer = buffer;
 _Bool ok = wlr_output_test_state(output, &copy);
...
_Bool wlr_output_configure_primary_swapchain(struct wlr_output *output,
 ...
 if (old_swapchain != ((void*)0) &&
   old_swapchain->width == width && old_swapchain->height == height &&
   old_swapchain->format.format == format) {
  return 1;
 }

 struct wlr_swapchain *swapchain = create_swapchain(output, width, height, format, 1);
 if (swapchain == ((void*)0)) {
  _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to create swapchain for output '%s'", "types/output/swapchain.c", 92, output->name);
  return 0;
 }

 _wlr_log(WLR_DEBUG, "[%s:%d] " "Testing swapchain for output '%s'", "types/output/swapchain.c", 96, output->name);
 if (!test_swapchain(output, swapchain, state)) {
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Output test failed on '%s', retrying without modifiers", "types/output/swapchain.c", 99, output->name);

  wlr_swapchain_destroy(swapchain);
  swapchain = create_swapchain(output, width, height, format, 0);
  if (swapchain == ((void*)0)) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "Failed to create modifier-less swapchain for output '%s'", "types/output/swapchain.c", 104, output->name);

   return 0;
  }
  _wlr_log(WLR_DEBUG, "[%s:%d] " "Testing modifier-less swapchain for output '%s'", "types/output/swapchain.c", 107, output->name);
  if (!test_swapchain(output, swapchain, state)) {
   _wlr_log(WLR_ERROR, "[%s:%d] " "Swapchain for output '%s' failed test", "types/output/swapchain.c", 109, output->name);
   wlr_swapchain_destroy(swapchain);
   return 0;
  }
 }
